...
const AdminManageGuide = () => {
    const [form, setForm] = useState({ title: '', image: null, fileURL: '' });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editGuideId) {
                await axios.put(`${API_BASE_URL}/api/guides/${editGuideId}`, formData);
            ...
            setForm({ title: '', image: null, fileURL: '' });
    ...
    return (
        <div>
            ...
            <div id="TTw" style={{ maxWidth: 500, margin: '10rem auto', padding: 20, background: '#fff', borderRadius: 8, boxShadow: '0 2px 8px #eee' }}>
                <h2>{editGuideId ? "Edit" : "Add"} Guide</h2>
                <form onSubmit={handleSubmit}>
                    <div style={{ marginBottom: 10 }}>
                        ...
                        {(form.image && !(form.image instanceof File)) ?
                            <div>
                                <img src={form.image} alt='guide' style={{ width: '100%', maxHeight: 350, marginBottom: 8 }} />
                    ...
                    <button id="insb" type="submit" disabled={loading} style={{ width: '100%', padding: 10, background: '#007bff', color: '#fff', border: 'none', borderRadius: 4 }}>
                        {loading ? (editGuideId ? "Updating..." : 'Adding...') : (editGuideId ? "Update Guide" : 'Add Guide')}
                    </button>
                </form>
            ...
            <div style={{ maxWidth: 700, margin: '2rem auto', padding: 20, background: '#fff', borderRadius: 8, boxShadow: '0 2px 8px #eee' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    ...
                    <tbody>
                        {guides.map((inst) => (
                            <tr key={inst._id}>
                                ...
                                <td style={{ padding: 8, border: '1px solid #ddd', display: "flex", gap: "8px" }}>
                                    <button onClick={() => handleEdit(inst)} style={{ background: editGuideId === inst._id ? "#6c757d" : '#ffc107', color: editGuideId === inst._id ? "#fff" : '#000', border: 'none', borderRadius: 4, padding: '6px 18px' }}>{editGuideId === inst._id ? "Cancel" : "Edit"}</button>
                        ...
                        {guides.length === 0 && (
                            <tr><td colSpan={3} style={{ textAlign: 'center', padding: 20 }}>No guides found.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
